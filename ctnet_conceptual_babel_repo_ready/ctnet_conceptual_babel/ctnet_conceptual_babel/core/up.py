...
import numpy as np
from .node import ConceptNode, normalize
from .relation import RelationOperator

class UPBundle:
    def __init__(self,d:int):
        self.d=d; self.half=d//2
        from .node import rng_for
        rng=rng_for("UPBundle")
        self.F=rng.normal(0.0,0.14,(self.half,self.half)); self.G=rng.normal(0.0,0.14,(self.half,self.half))
    ...
    def residual_node(self,node:ConceptNode,neighborhood:np.ndarray)->np.ndarray:
        x=normalize(0.82*node.state+0.18*neighborhood); u,p=self.split(x)
    ...
    def residual_relation(self,rel:RelationOperator,nodes:Dict[str,ConceptNode])->np.ndarray:
        ...
        src=nodes[rel.source].state; tgt=nodes[rel.target].state
        transported=normalize(rel.matrix@tgt); expected=normalize(src)
