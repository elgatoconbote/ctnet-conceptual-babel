...
def normalize(x:np.ndarray, eps:float=1e-8)->np.ndarray:
    n=np.linalg.norm(x)
    return x.copy() if n<eps else x/n

@dataclass
class ConceptNode:
    name:str; state:np.ndarray; complexity_level:int=0; charts:Dict[str,str]=field(default_factory=dict)
    memory_trace:float=0.0; potential_cardinality:str='unbounded'; metadata:Dict[str,Any]=field(default_factory=dict)
    ...
    def refine(self, context:str, max_children:int=3)->List['ConceptNode']:
        ...
        for idx,child_name in enumerate(self._child_names(context,max_children)):
            ...
            children.append(ConceptNode(child_name,child_state,self.complexity_level+1,{"spanish":self._spanish_projection(child_name),"symbolic":child_name},0.80*self.memory_trace,'unbounded',{"parent":self.name,"refinement_context":context}))
        return children
    ...
    @staticmethod
    def _spanish_projection(name:str)->str: return name.replace('_',' ')

@dataclass
class FractionNode(ConceptNode):
    fraction:str=''; chart_name:str='surface'
