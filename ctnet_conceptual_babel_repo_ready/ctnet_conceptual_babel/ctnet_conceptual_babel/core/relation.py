...
class RelationOperator:
    source:str; target:str; relation_type:str; matrix:np.ndarray; weight:float=1.0; expected_alignment:float=0.72; metadata:Dict[str,Any]=field(default_factory=dict)
...
def make_relation(source:str,target:str,relation_type:str,d:int,weight:float=1.0)->RelationOperator:
    ...
    return RelationOperator(source,target,relation_type,np.eye(d)+rng.normal(0.0,0.025,(d,d)),weight=weight)

def compose_relations(a:RelationOperator,b:RelationOperator)->RelationOperator:
    if a.target!=b.source: raise ValueError('Relation endpoints do not compose')
    return RelationOperator(a.source,b.target,f"{a.relation_type}∘{b.relation_type}",b.matrix@a.matrix,weight=(a.weight+b.weight)/2)

def project_relation(rel:RelationOperator,v:np.ndarray)->np.ndarray:
    return rel.matrix @ v
