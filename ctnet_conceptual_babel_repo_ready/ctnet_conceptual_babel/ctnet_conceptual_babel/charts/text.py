...
class TextChart:
    ...
    def concept_vector(self,name:str)->np.ndarray:
        rng=rng_for('concept::'+name); v=rng.normal(0.0,1.0,self.d); j=np.arange(1,self.d+1); phase=(stable_seed(name)%997)+1; v+=0.25*np.sin(j*phase/17.0)+0.15*np.cos(j*phase/31.0); return normalize(v)
    ...
    def _infer_goal(lower):
        ...
        return 'stabilize'
    def project(self,complex_,trace:Optional[Dict[str,Any]]=None)->str:
        ...
        if 'biblioteca_babel' in has and 'tensor_coherencia' in has and 'u_p' in has: core='La Biblioteca de Babel queda tratada como campo potencial de complejos conceptuales, no como lista de caracteres. u/p abre la tensión entre actuación y forma; el tensor de coherencia pesa esa tensión y curva el potencial para que aparezca un complejo proyectable.'
        elif 'nodo_conceptual' in has and 'relacion_infinita' in has: core='El centro activo es un complejo de nodos conceptuales relacionales. Cada nodo conserva una sección finita, pero su potencial queda abierto por expansión, relación, refinamiento y proyección.'
        elif 'realizacion_codigo' in has: core='La realización correcta es construir un runtime donde el texto solo sea una carta: se eleva a nodos, se estabiliza por energía u/p bajo H, y luego se proyecta en español.'
        if energy is not None and closure is not None: core += f' Energía final={energy:.4f}; cierre={closure:.4f}.'
        return core+' Nodos dominantes: '+', '.join(d.replace('_',' ') for d in dominant)+'.'
