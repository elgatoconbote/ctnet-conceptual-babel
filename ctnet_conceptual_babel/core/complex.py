...
class ConceptComplex:
    ...
    def clone(self) -> 'ConceptComplex':
        return ConceptComplex({k: v.clone() for k, v in self.nodes.items()}, [r.clone() for r in self.relations], self.regime, self.closure_goal, list(self.history), self.potential_cardinality)
